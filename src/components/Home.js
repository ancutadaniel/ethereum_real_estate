...
const Home = ({ home, provider, account, escrow, toggleProp }) => {
  ...
  const renderButton = (account) => {
    ...
    const { onClick, disabled, label } =
      buttonProps[account] || buttonProps.default;

    return (
      <button className='home__buy' onClick={onClick} disabled={disabled}>
        {label}
      </button>
    );
  };
